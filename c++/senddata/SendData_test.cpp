...
int main(int argc, char* argv[]) {
	...
	try {
		unsigned num_chanels=135;
		vector<std::string> channels(num_chanels,"Ch-");
		// make a new stream_info (100 Hz)
		unsigned sample_rate=1;
		lsl::stream_info info(name,type,num_chanels,sample_rate,lsl::cf_float32,string(name)+=type);

		// add some description fields
		info.desc().append_child_value("manufacturer", "BioSemi");
		lsl::xml_element chns = info.desc().append_child("channels");
		for (int k=0;k<num_chanels;k++)
